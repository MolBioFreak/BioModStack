...
workflow FAMPNN_CHILD {
    ...
    main:
        ...
        if (filterEnabled) {
            ...
            output_pdbs = RunFAMPNN.out.pdbs_jsons.map { it[0] }
            output_jsons = RunFAMPNN.out.pdbs_jsons.map { it[1] }
        }
...
workflow {
    ...
    def pdb_list = pdb_paths_raw.split(',')
    ...
    if (params.fampnn_max_psce) println("* Max pSCE: ${params.fampnn_max_psce}")
    if (params.fampnn_max_residue_psce) println("* Max residue pSCE: ${params.fampnn_max_residue_psce}")
    println("=" * 60)
