...
are empty placeholders. torch.distributed.run --standalone rendezvouses by name,
fails with `gai error: -3 Temporary failure in name resolution` — or, once
localhost resolves, on the container's own hostname — and no rank is ever
started: the GPUs stay idle while the workflow burns its budget.
"""
...
def test_container_gets_a_generated_loopback_hosts_file(tmp_path):
    bindings = driver.name_resolution_bindings(tmp_path, hostname='worker-1')
    ...
    assert source.read_text() == driver.hosts_file_content('worker-1')
...
def test_generated_hosts_maps_the_container_own_hostname(tmp_path):
    # The rendezvous endpoint is the host name when no MASTER_ADDR is set, so the
    # container's own name must resolve to loopback too.
    assert '127.0.0.1 %s\n' % os.uname().nodename in driver.hosts_file_content()
    bindings = driver.name_resolution_bindings(tmp_path, hostname='6db20091e47e')
    assert '127.0.0.1 6db20091e47e\n' in Path(bindings[0][0]).read_text()
