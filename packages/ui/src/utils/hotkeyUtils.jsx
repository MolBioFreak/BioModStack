...
export const withHotkeys = (hotkeys, handlers) => {
  return ({ children } = {}) => {
    const memoedHotkeys = useMemo(
      () => {
        const hotkeyConfigs = Object.keys(hotkeys).map(id => {
          const { ...props } = getHotkeyProps(hotkeys[id], id);
          console.log('[OVE Hotkeys] Registering:', id, props.combo);
          return {
            key: id,
            global: props.global !== false,
            onKeyDown: function (e) {
              console.log('[OVE Hotkeys] Triggered:', id);
              return handlers[id](e);
            },
            ...props
          };
        });
        console.log('[OVE Hotkeys] Total registered:', hotkeyConfigs.length);
