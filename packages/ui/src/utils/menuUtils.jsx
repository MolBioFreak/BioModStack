...
import { lifecycleHook as lifecycle, branchHook as branch } from "./recomposeCompat";
...
import {
  ...
  hideContextMenu as bpHideContextMenu,
  Menu,
  Classes,
  Icon
...
export const EnhancedMenuItem = compose(
  ...
  // React 19 compatible popover props - disable portal to avoid findDOMNode issues
  const react19PopoverProps = {
    autoFocus: false,
    usePortal: false, // Disable portal to avoid react-transition-group issues
  ...
  return (
    <MenuItemComp
      popoverProps={react19PopoverProps}
      {...(navTo && { navTo })}
      {...props}
      onClick={
        props.onClick
  ...
  if (item.divider !== undefined) {
    ...
    out = (
      <ItemComponent
        ...
        labelElement={item.hotkey && <KeyComboTag minimal combo={item.hotkey} />}
        text={item.text}
      >
        {item.submenu
          ? item.submenu
...
export function showContextMenu(
  ...
  // Render a context menu at the passed event's position using BP5 API
  bpShowContextMenu({
    content: (
      <MenuComponent>
