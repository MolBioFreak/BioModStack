/**
 * React 19 compatible MenuBar implementation
 * 
 * This replaces Blueprint's Popover which uses react-transition-group
 * and findDOMNode (removed in React 19) with a simple dropdown using
 * ReactDOM.createPortal and CSS positioning.
 */
import React, { createRef, createPortal } from "react";
...
import "./style.css";
import { Menu, Button, Classes } from "@blueprintjs/core";
...
// Simple dropdown wrapper that works with React 19
// Uses createPortal without react-transition-group
class SimpleDropdown extends React.Component {
  ...
  handleDocumentClick = (e) => {
    ...
    // Check if click is inside menu or target
    const menuEl = this.menuRef.current;
    const targetEl = this.targetRef.current;
  ...
  render() {
    const { isOpen, children, content, onClose } = this.props;
    ...
    const maxHeight = vh - ((position?.top || 70) - window.scrollY + 20);
    ...
    return (
      <>
        ...
        {isOpen && position && ReactDOM.createPortal(
          <div
            ref={this.menuRef}
            className="bp5-portal tg-simple-dropdown-portal"
            style={{
              position: 'absolute',
              top: position.top,
              left: position.left,
            ...
            onClick={(e) => {
              // Close on menu item click (unless explicit shouldDismissPopover: false)
              const target = e.target;
              if (target.closest('.bp5-menu-item, .bp3-menu-item')) {
                setTimeout(() => onClose?.(), 0);
              }
            }}
          >
            <div
              className="bp5-popover bp5-minimal"
              style={{ maxHeight, overflow: 'auto' }}
            >
              <div className="bp5-popover-content">
                {content}
              </div>
            </div>
          </div>,
...
class MenuBar extends React.Component {
  ...
  handleButtonClick = (index, topLevelItem) => () => {
    if (topLevelItem.onClick) {
        ...
        {menu.map((topLevelItem, i) => {
          ...
          const button = (
            <Button
              ...
              onClick={this.handleButtonClick(i, topLevelItem)}
          ...
          const menuContent = createDynamicMenu(
          ...
          return (
            <SimpleDropdown
              ...
              content={
                <Menu
                  style={
                    some(topLevelItem.submenu, n => n.isMenuSearch)
                      ? {}
