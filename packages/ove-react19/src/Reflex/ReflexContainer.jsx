...
class ReflexContainer extends React.Component {
  ...
  /////////////////////////////////////////////////////////////
  // React 19 compatible replacement for UNSAFE_componentWillReceiveProps
  // Checks if children or flex properties have changed and updates state
  /////////////////////////////////////////////////////////////
  componentDidUpdate(prevProps) {
    const prevChildren = this.getValidChildren(prevProps);
    const children = this.getValidChildren(this.props);

    // Check if children count changed or flex values changed
    if (
      children.length !== this.state.flexData.length ||
      this.flexHasChanged(this.props, prevProps)
    ) {
  ...
  //
  /////////////////////////////////////////////////////////
  flexHasChanged(currentProps, prevProps) {
    const currentChildrenFlex = this.getValidChildren(currentProps).map(child => {
    ...
    return !prevChildrenFlex.every((flex, idx) => {
      return flex === currentChildrenFlex[idx];
    });
  }
  ...
  getSize(element) {
    const ref = element.ref
      ? this.domRefs[element.ref]
      : element;

    const domElement = ref && ref.getDomNode ? ref.getDomNode() : ref;

    // Safety check if domElement is null/undefined
  ...
  emitElementsEvent(elements, event) {
    this.toArray(elements).forEach(element => {
      if (element.props[event]) {
        const ref = this.domRefs[element.ref];
        const domElement = ref && ref.getDomNode ? ref.getDomNode() : ref;
  ...
  render() {
    ...
    this.children = React.Children.map(
      ...
      (child, idx) => {
        ...
        const newProps = Object.assign({}, child.props, {
          ...
          ref: (el) => { this.domRefs[flexData.guid] = el; },
          index: idx
        });
    ...
    return (
      <div className={classNames.join(" ")} style={this.props.style} ref={el => this.domNode = el}>
        {this.children}
      </div>
    );
  }
