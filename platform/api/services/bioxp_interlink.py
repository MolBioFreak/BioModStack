...
def describe_state(*, recommended_url: str, runtime_note: str | None = None) -> dict[str, Any]:
    ...
    if runtime_note is None:
        if active:
            if _SESSION.get("reachable") is True:
                runtime_note = "Active operator-initiated BioXP interlink is reachable."
            elif _SESSION.get("reachable") is False:
    ...
    return {
        ...
        "reachable": _SESSION.get("reachable") if active else None,
        "hardware_connected": _SESSION.get("hardware_connected") if active else None,
