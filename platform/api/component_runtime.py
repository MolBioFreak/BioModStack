...
def _validate_native_compilation_fields(value) -> None:
    """One immutable settings/input validator for planning and execution."""
    if (type(value.model_id) is not str or not value.model_id
            or type(value.mode) is not str or not value.mode):
        raise ValueError('native compilation requires model and mode')
    if value.source_identity is not None and not isinstance(value.source_identity, SourceIdentity):
        raise ValueError('native invocation source identity must be typed')
    if value.entrypoint is not None:
        if type(value.entrypoint) is not str or '\x00' in value.entrypoint:
        ...
        if (entrypoint.is_absolute() or '..' in entrypoint.parts
                or '\\' in value.entrypoint or entrypoint.as_posix() != value.entrypoint
                or entrypoint.suffix != '.nf'):
            raise ValueError('native workflow entrypoint must be a contained Nextflow source path')
    if type(value.generated_inputs) is not tuple or any(
            not isinstance(item, GeneratedInput) for item in value.generated_inputs):
        raise ValueError('generated input roster must be immutable and typed')
    paths = {item.relative_path for item in value.generated_inputs}
    if len(paths) != len(value.generated_inputs):
    ...
    for payload in (value.requested_json, value.effective_json, value.native_parameters_json):
    ...
    if value.execution_plan is not None:
        if not isinstance(value.execution_plan, SelectedExecutionPlan):
            raise ValueError('native invocation execution plan must be typed')
        object.__setattr__(value, 'execution_plan', value.execution_plan.bind_invocation(value))


@dataclass(frozen=True)
class NativeInvocationPreview:
    """Read-only compiler projection: deliberately has no command or writer.

    The native compiler still owns normalization, generated bytes and the shared
    plan. Preview describes resource requirements, never physical reservations.
    It cannot be mistaken for an executable NativeInvocation.
    """
    model_id: str
    mode: str
    requested_json: bytes
    ...
    def __post_init__(self) -> None:
        _validate_native_compilation_fields(self)

    @classmethod
    def capture(cls, *, model_id: str, mode: str, requested: Mapping[str, Any],
                effective: Mapping[str, Any], native_parameters: Mapping[str, Any],
                entrypoint: str, generated_inputs: Sequence[GeneratedInput] = ()) -> NativeInvocationPreview:
        return cls(model_id, mode, canonical_bytes(dict(requested)), canonical_bytes(dict(effective)),
                   canonical_bytes(dict(native_parameters)), generated_inputs=tuple(generated_inputs),
