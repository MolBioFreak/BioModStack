...
def test_boltz_cp_production_sources_do_not_embed_local_four_gpu_default() -> None:
    ...
    for source_path in production_files:
        ...
        assert "0,1,2,3" not in source, f"{source_path} must use scheduler-provided GPU IDs"

    module_text = production_files[1].read_text(encoding="utf-8")
    config_text = production_files[0].read_text(encoding="utf-8")
    assert "params.get('bcp_gpu_ids', null)" in module_text
