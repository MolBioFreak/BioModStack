...
from experiment_services import canonical_json, save_hierarchy_revision
...
@pytest.mark.asyncio
async def test_frustrampnn_scope_accepts_the_global_revision_bound_to_the_domain_revision(
    read_model_store,
    core_store,
) -> None:
    ...
    assert response.status_code == 200, response.text
    assert response.json()["global_experiment_revision_id"] == experiment.current_revision_id
    assert response.json()["domain_revision_id"] == domain.current_revision_id
...
async def test_frustrampnn_scope_rejects_individually_valid_but_unrelated_revisions(
    ...
    async with factory() as session:
        project, experiment, domain = await _hierarchy(session)
        original_global_revision_id = experiment.current_revision_id
        domain_revision_id = domain.current_revision_id
        global_revision = await session.get(ExperimentRevision, original_global_revision_id)
        assert global_revision is not None
        payload = json.loads(global_revision.canonical_payload)
        payload["change_summary"] = "unrelated later Global revision"
        unrelated_global_revision = await save_hierarchy_revision(
            session,
            experiment.id,
            "experiment",
            payload,
...
@pytest.mark.asyncio
@pytest.mark.parametrize("mismatch", ["source_job_id", "content_digest"])
async def test_frustrampnn_scope_does_not_complete_a_foreign_scheduler_member(
    read_model_store,
    core_store,
    mismatch: str,
) -> None:
    ordinal = 7 if mismatch == "source_job_id" else 8
    design_id = f"selected-design-{ordinal}"
    ...
    membership_digest = f"{ordinal + 40:064x}"
    member_source_job_id = "foreign-source-job" if mismatch == "source_job_id" else source_job_id
    member_digest = f"{ordinal + 60:064x}" if mismatch == "content_digest" else membership_digest
    ...
    async with core_store() as session:
        session.add_all([
            Job(id=source_job_id, name=f"Source {ordinal}", status="completed", queue_status="completed", model_id="boltz2", mode="structure_prediction", params={}),
            Design(id=design_id, job_id=source_job_id, name=f"Structure {ordinal}", pdb_path=f"/fixture/{design_id}.pdb"),
            Job(
                id=child_id, name=f"FrustraMPNN {ordinal}", status="completed", queue_status="completed",
                model_id="frustrampnn", mode="analyze",
                params={"_frustrampnn_child_v1": {
                    ...
                    "selection": [{
                        "selection_ordinal": 0, "design_id": design_id,
                        "source_job_id": member_source_job_id, "candidate_id": candidate_id,
                        "invocation_id": invocation_id, "sha256": member_digest,
                    }],
                    "component_invocation_ids": [invocation_id],
                }},
                child_stage="frustrampnn", parent_job_id=source_job_id,
            ),
        ])
        manifest = {
            "schema_name": "frustrampnn_result_manifest", "parent_job_id": child_id,
            "invocation_id": invocation_id, "candidate_id": candidate_id,
            "request_sha256": f"{ordinal + 70:064x}", "source_sha256": member_digest,
        }
        summary = {"schema_name": "frustrampnn_summary", "candidate_id": candidate_id}
        session.add(FrustraMPNNResult(
            parent_job_id=child_id, invocation_id=invocation_id,
            parent_workflow_id="frustrampnn_analysis", candidate_id=candidate_id,
            design_id=design_id, requiredness="required", request_sha256=f"{ordinal + 70:064x}",
            source_artifact_id=design_id, source_artifact_sha256=member_digest,
            manifest_sha256=hashlib.sha256(canonical_json_bytes(manifest)).hexdigest(), manifest_json=manifest,
            summary_sha256=hashlib.sha256(canonical_json_bytes(summary)).hexdigest(), summary_json=summary,
            runtime_identity_json={}, assigned_gpu_json={}, terminal_result_json={"status": "completed"},
        ))
        await session.commit()
    ...
    assert item["reopen_uri"] is None
