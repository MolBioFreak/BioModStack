...
async def test_model_scope_summary_pagination_and_exact_lineage(tmp_path: Path):
    ...
    async with factory() as session:
        ...
        session.add_all([Design(id=f'a-{i:03}', job_id='parent', name=f'a-{i:03}', pdb_path=f'{i}.pdb', producer_model_id='boltz2', provenance={'model_id': 'boltz2'}) for i in range(501)])
        session.add_all([Design(id=f'z-{i}', job_id='child', name=f'z-{i}', pdb_path=f'z-{i}.pdb', producer_model_id='protenix', provenance={'model_id': 'protenix'}, plddt_overall=70 + i) for i in range(3)])
        session.add(Design(id='foreign-design', job_id='foreign', name='foreign', pdb_path='foreign.pdb', producer_model_id='protenix', provenance={'model_id': 'protenix'}))
