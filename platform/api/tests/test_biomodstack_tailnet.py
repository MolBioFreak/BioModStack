...
def test_host_listener_owner_helper_uses_stable_pinned_python_image(monkeypatch) -> None:
    ...
    command = commands[0]
    assert command[command.index("--entrypoint") + 1] == "/usr/local/bin/python3.10"
    assert tailnet.HOST_PROC_HELPER_IMAGE in command
...
def test_canonical_environment_root_requires_exact_clean_durable_branch(tmp_path: Path) -> None:
    ...
    root.mkdir()
    subprocess.run(["git", "init", "-q", "-b", "test", str(root)], check=True)
    ...
    subprocess.run(["git", "-C", str(root), "update-ref", "refs/remotes/origin/test", head], check=True)

    assert tailnet._validate_canonical_environment_root(root, "development") == head

    (root / "tracked.txt").write_text("dirty\n", encoding="utf-8")
    with pytest.raises(tailnet.TailnetEnvironmentError, match="not clean"):
...
def test_operator_development_frontend_is_pinned_to_isolated_dev_api(monkeypatch, tmp_path: Path) -> None:
    systemd_dir = tmp_path / "systemd"
    monkeypatch.setattr(tailnet, "_host_user_systemd_dir", lambda: systemd_dir)
    monkeypatch.setattr(
        tailnet,
...
def test_host_owner_helper_uses_kernel_proc_names_not_human_ls_output(monkeypatch) -> None:
    ...
    assert captured[captured.index("--entrypoint") + 1] == "/usr/local/bin/python3.10"
    assert tailnet.HOST_PROC_HELPER_IMAGE in captured
