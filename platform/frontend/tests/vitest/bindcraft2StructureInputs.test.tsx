...
import { acquireBC2Source, editVisualResidues, fastaChains, parseBC2Document, readBC2Source, residueKey, selectedChains, sliceBC2Structure, sourceDownloadUrl, visualResidues, type BC2InitialSources } from '../../src/lib/bindcraft2StructureInputs';
...
describe('mounted BC2 visual preparation workspace', () => {
    ...
    it('selects a prior-run design through real JobBrowser and materializes exactly its downloaded bytes', async () => {
        ...
        await settle(() => uploaded.length === 1);
        expect(uploaded[0].content).toBe(pdb); expect((request.targets as any[])[0].target_path).toBe(uploaded[0].path);
    });
