...
const state=vi.hoisted(()=>({props:{} as any,gpu:{} as any}));
...
vi.mock('../../src/components/MolstarViewerImpl',()=>({default:(props:any)=>{state.gpu=props;return <div data-structure-visible/>;}}));
vi.mock('../../src/structureViewer/extensions/m6/M6WorkbenchPanel',()=>({M6WorkbenchPanel:()=>null}));
vi.mock('../../src/structureViewer/StructureWorkbench',()=>({StructureWorkbench:(props:any)=>{state.props=props;return <StructureViewerHost {...props}/>;}}));
...
async function mount(residue=wire.residue,chains=wire.chains){
 vi.stubGlobal('fetch',vi.fn(async(url:string)=>({ok:true,json:async()=>url.endsWith('/residue-metrics')?residue:url.includes('/pae?')?wire.pae:chains})));
...
afterEach(async()=>{if(mounted)await act(async()=>mounted!.unmount());mounted=undefined;client.clear();vi.unstubAllGlobals();});
it('native vectors and provider-chain map survive real transport into shared bounded track/exact selection',async()=>{
 ...
 await mount();
 expect(mounted!.root.findAllByProps({'aria-label':'Native chain metrics'})).toHaveLength(0);
 await act(async()=>mounted!.root.findAllByType('button').find(el=>text(el)==='Show metrics')!.props.onClick());
 ...
 expect(mounted!.root.findAllByProps({'aria-label':'Native residue confidence'})).toHaveLength(0);

 const selected=residue.residues[3];
 const label=`${selected.labelAsymId??selected.authAsymId}:${selected.labelSeqId??selected.authSeqId}${selected.insertionCode??''}:`;
 const point=mounted!.root.findByProps({'aria-label':'Linked sequence track'}).findAllByProps({role:'option'}).find(el=>el.props.title.startsWith(label))!;
 expect(text(point)).toContain(`${(wire.residue.values[3]*100).toFixed(1)}%`);
 await act(async()=>point.props.onClick());
 expect(state.gpu.scenePresentation.colorQueries.filter((q:any)=>q.focus)).toEqual([expect.objectContaining({authAsymId:selected.authAsymId,startAuthSeqId:selected.authSeqId,endAuthSeqId:selected.authSeqId,insertionCode:selected.insertionCode})]);
 expect(state.gpu.scenePresentation.colorQueries.at(-1).focus).toBe(true);
 ...
 expect(state.gpu.scenePresentation.selection).toEqual([]);
