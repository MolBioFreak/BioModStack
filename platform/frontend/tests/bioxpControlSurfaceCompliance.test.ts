...
test('operator surface is compact and robot-authority driven', () => {
  ...
  for (const label of ['BioXP 3200', 'Connection', 'Controller Activation & Recovery', 'Activate 24 V / Prepare Motion', 'Non-homing Recovery', 'Manual Controls', 'BioXpCameraPanel', 'Software Abort', 'Recent Robot Actions']) assert.match(source, new RegExp(label));
...
test('operator surface excludes rejected planning and evidence UI', () => {
  ...
  assert.match(source, /X initialization writes neither register/);
