...
describe('molecular dynamics launcher contract', () => {
    it('builds an authoritative engine-neutral bms.md.job.v1 specification', () => {
        ...
        assert.equal(spec.stages.production.trajectory_interval_steps, 5_000);
        assert.equal(spec.stages.production.energy_interval_steps, 1_000);
    ...
    it('reports aggregate simulation scope without inventing throughput', () => {
        assert.deepEqual(estimateMolecularDynamicsScope(validForm()), {
            ...
            trajectoryFramesPerReplica: 0,
            totalTrajectoryFrames: 0,
        });
    });
});
