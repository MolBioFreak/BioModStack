...
test('BMS DB service top-bar menu and control panel expose runtime contract', () => {
  ...
  assert.doesNotMatch(controlSource, /State, actions, logs\./);
  ...
  assert.doesNotMatch(controlSource, /BMS DB spec/);
  assert.doesNotMatch(controlSource, /PostgreSQL docs/);
  assert.doesNotMatch(controlSource, /SQLite docs/);
