...
import type { TelemetryChartHistoryResponse } from '../src/lib/telemetryChart';
import type { LiveSample } from '../src/components/infraTelemetryHistory';
...
test('mounted CPU, RAM and GPU panels pass raw units and preserve absent GPU readings', async () => {
    ...
    try {
        ...
        assert.ok(lines.every(line => !('customdata' in line)), 'SVG hover reads raw y values, not duplicate arrays');
        assert.ok(plots[0].props.samples.every((sample: LiveSample) => !('clock' in sample)), 'unused sample labels are not allocated');
    } finally { await act(async () => root!.unmount()); client.clear(); }
});


test('non-history rerenders reuse samples; changed bounds filter without rebuilding history', async () => {
    const client = new QueryClient({ defaultOptions: { queries: { enabled: false, retry: false } } });
    const key = ['compact-telemetry-chart-history', 1, 1000, 1000];
    const point = { timestamp_ms: 1000, sample_count: 1, cpu_utilization: 12,
        cpu_frequency_current_mhz: 3250, cpu_power_watts: 150, cpu_temperature: 55,
        ram_used_gb: 12, ram_available_gb: 20, ram_utilization: 37.5, ram_swap_percent: 2, gpus: [] };
    const history = { start_ms: 0, end_ms: 4000, generated_at_ms: 4000,
        next_cursor_ms: 3000, bucket_ms: 1000,
        points: [point, { ...point, timestamp_ms: 2000 }, { ...point, timestamp_ms: 3000 }] };
    client.setQueryData(key, history);
    let root: ReactTestRenderer;
    const render = (labels: boolean) => <QueryClientProvider client={client}>
        <InfraLiveTelemetry defaultWindowMinutes={1} showXAxisLabels={labels} />
    </QueryClientProvider>;
    await act(async () => { root = create(render(true)); });
    const plot = () => root!.root.findAllByType(TimeSeriesPlot)[0];
    const updateHistory = async (changes: Partial<TelemetryChartHistoryResponse>) => {
        await act(async () => {
            client.setQueryData(key, (previous: TelemetryChartHistoryResponse) => ({ ...previous, ...changes }));
            await new Promise(resolve => setTimeout(resolve, 0));
        });
    };
    try {
        const samples = plot().props.samples;
        assert.equal(samples.length, 3);
        await act(async () => root!.update(render(false)));
        assert.equal(plot().props.showXAxisLabels, false, 'the panel really rerendered');
        assert.strictEqual(plot().props.samples, samples, 'presentation rerenders do not reconstruct or refilter history');

        await act(async () => {
            client.setQueryData(['powerControl'], { data: { limits: { 0: 250 } } });
            await new Promise(resolve => setTimeout(resolve, 0));
        });
        assert.strictEqual(plot().props.samples, samples, 'control updates do not reconstruct history');
        await updateHistory({ generated_at_ms: 5000 });
        assert.strictEqual(plot().props.samples, samples, 'response metadata alone is not new history');

        await updateHistory({ start_ms: 2000 });
        const filtered = plot().props.samples;
        assert.notStrictEqual(filtered, samples, 'changed domain bounds refilter history');
        assert.deepEqual(filtered.map((sample: LiveSample) => sample.timestampMs), [2000, 3000]);
        assert.strictEqual(filtered[0], samples[1], 'domain movement reuses reconstructed sample objects');
        assert.deepEqual(plot().props.xDomain, [2000, 3000], 'fresh endpoint/domain alignment stays current');

        await updateHistory({ points: [...history.points, { ...point, timestamp_ms: 4000, cpu_power_watts: 175 }], next_cursor_ms: 4000 });
        assert.deepEqual(plot().props.samples.map((sample: LiveSample) => sample.timestampMs), [2000, 3000, 4000]);
        assert.equal(plot().props.samples.at(-1).cpuPower, 175, 'new history invalidates the memo');
        assert.deepEqual(plot().props.xDomain, [2000, 4000]);
    } finally { await act(async () => root!.unmount()); client.clear(); }
});
