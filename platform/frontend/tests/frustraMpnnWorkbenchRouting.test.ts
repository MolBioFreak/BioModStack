...
test('direct and conformational-mapping results mount the same global FrustraMPNN workbench', () => {
    ...
    assert.match(direct, /aria-label="Result model"/);
    assert.match(direct, />Workflow output<\/button>/);
    assert.match(direct, />FrustraMPNN<\/button>/);
    assert.match(direct, /activeJob\.model_id !== 'frustrampnn'[\s\S]*>Workflow output<\/button>/);
    ...
    assert.match(resultSurface, /getGlobalExperiment\(/);
    assert.match(resultSurface, /getDomainExperiment\(/);
    assert.match(resultSurface, /fetchDomainFrustraMpnnResults\(/);
    assert.match(resultSurface, /globalHead\.data\.current_revision_id/);
    assert.match(resultSurface, /domainHead\.data\.current_revision_id/);
    assert.match(resultSurface, /enabled: hierarchyAuthorityReady/);
    assert.match(resultSurface, /Exact whole-experiment hierarchy authority is unavailable/);
    assert.match(resultSurface, /Whole-experiment hierarchy could not be loaded/);
    assert.match(resultSurface, /Whole-experiment results are loading/);
