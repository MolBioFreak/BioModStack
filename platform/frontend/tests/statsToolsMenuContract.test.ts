...
test('stats-tools lifecycle controls live in shared control panel and Stats Toolkit debug panel', () => {
  ...
  assert.match(controlSource, /BMS stats plan/);
  assert.match(controlSource, /https:\/\/github\.com\/MolBioFreak\/BioModStack\/blob\/main\/docs\/reports\/2026-05-05-bms-workflow-stats-tools-containerization-plan\.md/);
  assert.match(controlSource, /R Project/);
  assert.match(controlSource, /https:\/\/www\.r-project\.org\//);
  assert.match(controlSource, /Plotly docs/);
  assert.match(controlSource, /https:\/\/plotly\.com\/javascript\//);
