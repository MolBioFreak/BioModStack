...
test('OEM operator surface is connection, initialize, finite controls, camera and stop', () => {
  for (const label of ['Connection', 'Initialize Controllers', 'Manual Controls', 'BioXpCameraPanel', 'Emergency Stop']) assert.match(source, new RegExp(label));
});
