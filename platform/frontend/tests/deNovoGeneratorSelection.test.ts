...
test('saved native generator mode restores its own route and unknown selectors refuse substitution', () => {
    ...
    assert.equal(resolveExistingDeNovoGenerator({ denovo_generator: 'bindcraft2', mode: 'campaign' }), 'bindcraft2');
    assert.equal(resolveExistingDeNovoGenerator({ model_id: 'bindcraft2', mode: 'campaign' }), 'bindcraft2');
