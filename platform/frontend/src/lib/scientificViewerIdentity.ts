...
export type ScientificPae = { status: 'unavailable'; reason: string } | {
    status: 'ok'; reason: null; rows: AtomRef[]; columns: AtomRef[];
...
function axis(value: unknown, expected: StructureDocumentRef, binding: Record<string, unknown>, count: number): AtomRef[] {
...
export function parseScientificPae(raw: unknown, expected: StructureDocumentRef | null | undefined, selectedCandidateId = expected?.candidateId): ScientificPae {
    try {
        ...
        const bySourceIndex = new Map(rawRows.map((r, i) => [r.index, canonicalSpatialRefKey(rowAxis[i])]));
        if (nr !== nc || rawColumns.some((r, i) => bySourceIndex.get(r.index) !== canonicalSpatialRefKey(columnAxis[i]))) throw Error('contradictory axes');
