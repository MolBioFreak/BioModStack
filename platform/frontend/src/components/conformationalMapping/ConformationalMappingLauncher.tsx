...
const SOURCE_KINDS: Array<{ value: CmSourceKind; label: string; accept: string }> = [
    ...
    { value: 'structure_upload', label: 'Structure upload', accept: '.cif,.mmcif,.pdb' },
    { value: 'structure_artifact', label: 'Structure artifact', accept: '.cif,.mmcif,.pdb' },
...
export function ConformationalMappingLauncher({ onBack, initialValues }: Props) {
    ...
    const structureSources = useMemo(
        () => (sources.data || []).filter((source) => ['structure_upload', 'structure_artifact'].includes(source.source_kind)),
        [sources.data],
    );
    ...
    const validationErrors = useMemo(() => {
        ...
        if (form.backend === 'external_import') {
            if (!form.snapshotId) errors.push('Select the matching ordered complete-complex snapshot bundle.');
            if (!form.importIds.length) errors.push('Select at least one registered structure handle.');
            if (new Set(form.importIds).size !== form.importIds.length) errors.push('Ordered import handles must be unique.');
            if (selectedSnapshot && snapshotTargetIds.length && snapshotTargetIds.length !== form.importIds.length) {
                errors.push('Snapshot target count must equal the ordered import count.');
            }
        }
        ...
        return errors;
    }, [expectedCount, form, seedValues, selectedSnapshot, snapshotTargetIds.length, stepValues]);
    ...
    const buildPayload = (): CmSubmitRequest => {
        ...
        if (form.backend === 'protenix_v2_ensemble') payload.registered_snapshot_id = form.snapshotId;
        if (form.backend === 'external_import') {
