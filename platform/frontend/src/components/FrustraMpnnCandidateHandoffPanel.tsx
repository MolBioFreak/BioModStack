    ...
    return (
        <section aria-label="FrustraMPNN external candidate handoff" className="rounded-xl border border-amber-500/30 bg-amber-950/10 p-4">
            ...
            <fieldset disabled={mutation.isPending} className="mt-3 grid gap-3 md:grid-cols-2">
                <label className="text-xs text-slate-400">Candidate ID<input value={candidateId} onChange={(event) => { mutation.reset(); setCandidateId(event.target.value); }} className="mt-1 w-full rounded border border-slate-700 bg-slate-950 px-2 py-1.5 text-slate-200" placeholder="variant-1" /></label>
                <label className="text-xs text-slate-400">Producer ID<input value={producerId} onChange={(event) => { mutation.reset(); setProducerId(event.target.value); }} className="mt-1 w-full rounded border border-slate-700 bg-slate-950 px-2 py-1.5 text-slate-200" placeholder="external-redesign" /></label>
            ...
            {mutation.data && <div className="mt-3 rounded border border-emerald-500/30 bg-emerald-500/10 p-2 text-xs text-emerald-100">
                <div>Child queued: <span className="font-mono">{mutation.data.child_job_id}</span>. Submission receipt, not scientific completion.</div>
                {onOpenJob && <button type="button" onClick={() => onOpenJob(mutation.data!.result_job_id)} className="mt-2 underline">Open child results</button>}
