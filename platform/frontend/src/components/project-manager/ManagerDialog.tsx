...
interface ProteinCapabilityOption { capabilityId: string; label: string; scientificRole: string; allowedDomainModes: string[]; validatorDomainModes: string[] }
...
function parseProteinCapabilityInventory(value: unknown): ProteinCapabilityOption[] {
    ...
    return inventory.capabilities.flatMap((value) => {
        ...
        const validatorDomainModes = Array.isArray(row.validator_domain_modes)
            ? row.validator_domain_modes.filter((mode): mode is string => typeof mode === 'string' && mode.length > 0)
            : [];
        return [{
            capabilityId: row.capability_id,
            label: row.label,
            scientificRole: row.scientific_role,
            allowedDomainModes: row.allowed_domain_modes as string[],
            validatorDomainModes,
        }];
    });
}
...
export function ManagerDialog({ mode, projectId, summary, onClose, onComplete }: ManagerDialogProps) {
    ...
    const proteinCapabilitiesQuery = useQuery({
        ...
        queryFn: ({ signal }) => listProteinProjectCapabilities(signal).then(parseProteinCapabilityInventory),
        enabled: (mode === 'create_domain' && domainKind === 'protein_in_silico')
            || (mode === 'edit' && selection?.node_type === 'domain_experiment' && jsonRecord(detailQuery.data?.payload?.domain_payload)?.schema === 'bms.protein-in-silico-experiment.v3'),
        retry: false,
    });
    ...
    const validationCapabilityOptions = (proteinCapabilitiesQuery.data ?? []).filter((capability) => capability.validatorDomainModes.includes(proteinExperimentMode));
