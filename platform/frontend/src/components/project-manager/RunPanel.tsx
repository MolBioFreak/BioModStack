...
interface RunPanelProps {
    ...
    onLoadMore?: () => void;
    pendingAction?: { runId: string; action: string } | null;
...
function pendingLabel(action: string): string {
    return ({ open_results: 'Opening results…', clone: 'Cloning…', lineage: 'Opening lineage…', retry: 'Retrying…', resubmit: 'Resubmitting…' } as Record<string, string>)[action] ?? `${displayLabel(action)}…`;
}

export function RunPanel({ runs, selectedNodeKey, onSelect, onAction, onLoadMore, pendingAction, actionsDisabled }: RunPanelProps) {
    return (
        <section aria-label="Workflow runs" className="shrink-0 border-t border-border-primary bg-surface-secondary">
            <header className="flex items-center justify-between gap-3 px-4 py-3">
                <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.18em] text-content-muted">Execution evidence</p>
                    <h2 className="mt-0.5 text-sm font-semibold text-content">Workflow runs</h2>
                </div>
                <span className="rounded-full border border-border-primary px-2 py-1 text-xs text-content-secondary">{runs.length} bounded runs</span>
            </header>
            <div className="max-h-80 overflow-auto border-t border-border-primary">
                {!runs.length ? (
                    ...
                    <article key={run.run_id} aria-busy={pendingAction?.runId === run.run_id || undefined} className="border-b border-border-primary px-4 py-3 last:border-b-0">
                        <button
                            type="button"
                            ...
                            <span className="flex flex-wrap items-start justify-between gap-2">
                                ...
                                <span className="flex flex-wrap items-center justify-end gap-2">
                                    {run.replica_index !== null && (
                                        <span className="rounded-full border border-border-primary px-2 py-0.5 text-xs text-content-secondary">Scientific replica {run.replica_index}</span>
                                    )}
                                    <span className="rounded-full border border-border-primary px-2 py-0.5 text-xs text-content-secondary">Canonical {displayLabel(run.canonical_state)}</span>
                                    <span className="rounded-full bg-accent/15 px-2 py-0.5 text-xs font-semibold text-accent">Global {displayLabel(run.normalized_state)}</span>
                                </span>
                            </span>
                        </button>
                        ...
                        {run.condition.severity !== 'none' && (
                            <p role="status" className="mt-3 rounded-lg border border-border-primary bg-surface px-3 py-2 text-xs text-content-secondary">
                        ...
                        {run.available_actions.length > 0 && (
                            <div className="mt-3 flex flex-wrap gap-2" aria-label={`Actions for run ${run.run_id}`}>
                                {run.available_actions.map((action) => (
                                    <button key={action} type="button" disabled={Boolean(pendingAction) || actionsDisabled} onClick={() => onAction(action, run)} className="rounded-md border border-accent px-2.5 py-1.5 text-xs font-semibold text-accent focus:ring-2 focus:ring-accent disabled:cursor-wait disabled:opacity-50">
                                        {pendingAction?.runId === run.run_id && pendingAction.action === action ? pendingLabel(action) : displayLabel(action)}
                                    </button>
                                ))}
                            </div>
                        )}

                        {pendingAction?.runId === run.run_id && <p role="status" className="mt-2 text-xs text-accent">{pendingLabel(pendingAction.action)} Actions will become available when this finishes.</p>}
                        <details className="mt-3 text-xs text-content-secondary"><summary className="cursor-pointer font-semibold">Technical details and attempt history</summary>
                        <dl className="mt-2 grid gap-2 break-all">
                            <div><dt>Run / group</dt><dd>{run.run_id} · {run.batch_or_run_group_id ?? 'No run group'}</dd></div>
                            <div><dt>Canonical job</dt><dd>{run.canonical_job_id ?? 'Not materialized'}</dd></div>
                            <div><dt>Adapter</dt><dd>{run.adapter_id ?? 'Unavailable'}</dd></div>
                        </dl>
                        {run.attempts.length > 0 && (
                            <section aria-label={`Attempts for run ${run.run_id}`} className="mt-3">
                                <h3 className="text-xs font-semibold uppercase tracking-[0.14em] text-content-muted">Attempt provenance</h3>
                                    ...
                                    {run.attempts.map((attempt) => (
                                        <div key={attempt.attempt_id} className="rounded-lg border border-border-primary bg-surface p-3">
                                            <div className="flex items-center justify-between gap-2">
                                                ...
                                                <span className="text-xs text-content-secondary">{displayLabel(attempt.canonical_state)}</span>
                                            </div>
