...
export function RemotePreloadPanel({ target, jobs, onChanged }: Props) {
  ...
  return <section aria-label="Remote preload and activity" className="space-y-3 rounded-lg border border-border p-3">
    <h4 className="font-medium">Preload source and runtime files</h4>
    <p className="text-xs text-muted-foreground">Use a saved Job as the exact dependency recipe. This does not submit a Job or transfer biological inputs, results, or secrets. Downloads ready does not mean scientific Ready.</p>
    <label className="block text-sm">Saved Job recipe
      <select aria-label="Saved Job recipe" value={jobId} disabled={busy} onChange={event => setJobId(event.target.value)} className="mt-1 block w-full rounded border border-border bg-background p-2">
        <option value="">Select a saved Job</option>
        {jobs.map(job => <option key={job.id} value={job.id}>{job.name || job.id} — {job.model_id}</option>)}
      </select>
    </label>
    <button type="button" disabled={!canPreload || !validRecipe} onClick={() => void submit()} className="rounded border border-border px-3 py-1.5 text-sm disabled:opacity-50">
    ...
    {preload && <div role="status" aria-label="Preload progress" className="text-sm">
      ...
      <p className="text-xs text-muted-foreground">Recipe {preload.job_id} · Source {preload.source_revision.slice(0, 12)} · Updated {preload.updated_at}</p>
    </div>}
    {progress && <div role="status" aria-label="Worker activity" className="text-sm">
      ...
      <p className="text-xs text-muted-foreground">Job {progress.job_id} · Updated {progress.updated_at}</p>
    </div>}
  </section>;
}
