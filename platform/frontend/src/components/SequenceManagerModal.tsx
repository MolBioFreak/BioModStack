...
export function SequenceManagerModal({
    ...
    useEffect(() => {
        if (isOpen && initialSequence && mode === 'list') {
            setFormSequence(initialSequence.toUpperCase().replace(/[^A-Z]/g, ''));
            setFormName(initialName);
            setMode('edit');
        }
    }, [isOpen, initialSequence, initialName]);
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-semibold text-slate-200 text-lg">
                                {mode === 'list' ? 'Sequence Library' : editingSequence ? 'Edit Sequence' : 'New Sequence'}
                            </h3>
                            <p className="text-xs text-slate-500">
                                {mode === 'list' ? 'Manage your saved amino acid sequences' : 'Configure sequence details'}
                            </p>
                        </div>
                    </div>
                    <button
