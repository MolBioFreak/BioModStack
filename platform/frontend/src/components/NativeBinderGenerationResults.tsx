...
const values = (row: NativeGenerationRecord): Record<string, unknown> => row.metrics && typeof row.metrics === 'object' && !Array.isArray(row.metrics) ? row.metrics as Record<string, unknown> : {};
    ...
    const table = <section className={`${panel} overflow-hidden`} aria-label="Candidate data table">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border-color)] p-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
                ...
                <details className="relative"><summary className={`${control} cursor-pointer`}>Columns ({tableKeys.length})</summary><div className="absolute right-0 z-20 mt-2 max-h-72 w-64 overflow-auto rounded-lg border border-[var(--border-color)] bg-[var(--bg-primary)] p-3 shadow-xl">
                    ...
                    {keys.map(key => <label key={key} className="flex gap-2 py-1 text-sm" title={key}><input aria-label={`Column ${key}`} type="checkbox" checked={tableKeys.includes(key)} onChange={event => setColumns(event.target.checked ? [...tableKeys, key] : tableKeys.filter(item => item !== key))} />{metricLabel(key)}</label>)}
                </div></details>
            </div>
        </div>
        <div className="max-h-[520px] overflow-auto">
            <table className="w-full border-collapse text-sm"><thead className="sticky top-0 z-10 bg-[var(--bg-primary)] text-left"><tr>
                ...
                {tableKeys.map(key => <th key={key} className="whitespace-nowrap px-4 py-3 text-right" title={key} aria-sort={sort.key === key ? sort.descending ? 'descending' : 'ascending' : 'none'}><button type="button" aria-label={`Sort by ${key}`} onClick={() => toggleSort(key)}>{metricLabel(key)} {sort.key === key ? sort.descending ? '↓' : '↑' : '↕'}</button></th>)}
                ...
                {tableKeys.map(key => <td key={key} className="whitespace-nowrap px-4 py-3 text-right tabular-nums" title={nativeText(entry.values[key])}>{formatMetric(entry.values[key])}</td>)}
            </tr>; })}</tbody></table>
            ...
            {page.total === 0 ? <p className={`${panel} p-5`}>No native candidate records were emitted. This published zero-yield result remains available for review.</p> : <>
                <div className="grid grid-cols-2 gap-3 lg:grid-cols-4" aria-label="Cohort summary">
                    {[
                        ...
                        ...headlineMetrics.map(metric => [`Median ${metricLabel(metric.key)}`, metric.median == null ? '—' : formatMetric(metric.median), `${metric.observed} observations in this view`]),
                ...
                <div className={`${panel} space-y-3 p-3`} aria-label="Cohort controls">
                    ...
                    {filters.map((filter, index) => <div key={index} className="flex flex-wrap items-center gap-2 text-sm">
                        <select aria-label={`Filter ${index + 1} metric`} className={`${control} max-w-full`} value={filter.key} onChange={event => filterChanged(filters.map((item, i) => i === index ? { ...item, key: event.target.value } : item))}>{keys.map(key => <option value={key} key={key}>{metricLabel(key)} ({key})</option>)}</select>
                ...
                <div ref={inspector} hidden={view !== 'structure'} className={`${panel} p-4`} aria-label="Candidate structure inspector">
                    {!row ? <p role="status">{!complete ? 'Loading the requested candidate from the publication. ' : 'Requested candidate is unavailable in this publication. '}Another candidate is not substituted.</p> : <>
                        ...
                        </div><aside className="min-w-0"><h5 className="mb-2 text-sm font-semibold">Native measurements</h5><dl className="max-h-[600px] overflow-auto text-sm">{Object.entries(values(row)).map(([key, value]) => <div key={key} className="flex justify-between gap-3 border-b border-[var(--border-color)] py-2" title={key}><dt className="break-words text-[var(--text-secondary)]">{metricLabel(key)}</dt><dd className="max-w-[55%] break-words text-right tabular-nums">{formatMetric(value)}</dd></div>)}</dl></aside></div>
