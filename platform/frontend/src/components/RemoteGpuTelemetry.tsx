...
export function RemoteGpuTelemetry() {
    ...
    const metric = (title: string, suffix: string, value: (s: RemoteTelemetrySample) => number | null | undefined, maximum?: number) => (
        <TimeSeriesPlot key={title} height={120} samples={[]} xDomain={[Date.now() - 3_600_000, Date.now()]}
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-700 bg-slate-900 p-4">
                <div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
                ...
                {[...new Set(history.flatMap(s => (s.network ?? []).map(n => n.interface)))].map(name => <div key={name} className="space-y-2">
