                                ...
                                {selected.inputs.map((input) => (
                                    <label key={input.name} className="text-sm">
                                        ...
                                        {input.value_type === 'boolean' ? (
                                            ...
                                            <input type={input.value_type === 'string' ? 'text' : 'number'} value={String(inputs[input.name] ?? '')} min={input.minimum ?? undefined} max={input.maximum ?? undefined} onChange={(event) => setInputs((current) => ({ ...current, [input.name]: event.target.value }))} className="mt-1 w-full rounded border border-slate-700 bg-slate-900 p-2" />
                                        )}
