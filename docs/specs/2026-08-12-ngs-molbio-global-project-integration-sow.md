...
Release acceptance compares this inventory with the mounted toolkits, workflows, mutation routes, scheduler entrypoints, receipt resolvers, and result surfaces. These engineering checks are not operational admission. Runtime discovery must not revalidate the whole source package or historical Phase N0 receipts, require fixed capability/adapter/Dataset counts, or depend on the unrelated Protein constraint registry remaining empty. Discovery retains unique IDs and catalog integrity; an operation resolves its own registered schemas, referenced dependencies, native owners, and typed scientific settings. A broken unrelated entry must not disable otherwise valid operations. Authorization, execution ownership, and exact scientific input/output integrity remain mandatory.

### 3.1 Native operation and presentation boundaries

- Resource observations are optional telemetry, including signal-alignment completion. Missing, incomplete, malformed, or failed observations report unavailable; they do not invalidate native scientific outputs. Observations never substitute for actual execution ownership.
- Restriction recognition/cleavage analysis does not depend on supplier-product retrieval or redistribution approval. Supplier information loads independently and may be unavailable. Selected enzymes are immediately analyzable; full-catalog analysis is an explicit, progressive operation that retains successful batches and identifies incomplete scans.
- Saved alignments resolve exact immutable inputs and compute outside SQLite write transactions. The short atomic save retains idempotency and checks for a concurrent identical save after acquiring the writer.
- Gibson and DNA Weaver computation runs off the API event loop. Server-generated results are retained in the existing Mol Bio operation store, bound to their requested settings, immutable inputs when selected, and result digest. The typed browser/agent response exposes `computation_id`; Save uses that retained result and checks the selected candidate/plan checksum without replanning. No expiring preview receipt or separate result store is introduced. A computed preview does not itself create a saved library product.
- Assembly lineage resolves the selected immutable source revision and validates its sequence, orientation, and slice. Mutable display names and later head revisions do not invalidate that revision or silently replace it.
- Operational reads do not repeat release engineering or whole-history integrity audits. Source-package verification is reused for one immutable deployment-record generation, with an explicit fresh attestation at deployment/release boundaries. Registered schemas and compiled validators are reused; identical in-process status/receipt objects are not rehashed by each caller.
- Ordinary `/api/health` checks operational/schema readiness without walking retained scientific records or artifact bytes. `?deep=true`, startup, release and restore retain their full integrity checks. Skipped data checks are explicitly reported as not performed, not as freshly verified zero-error results. Readiness composes the owning service's status rather than independently rebuilding its policy and route receipt.
- Collection limits bound database projection and enumeration work, not just response slicing. Navigation summaries are distinct from exact scientific detail. Cursor continuation and total/partial indicators remain available; a large section must not reject the entire Project Hub. Editable completion handlers retain request/workspace/edit-generation ownership so a late result cannot erase newer work.
