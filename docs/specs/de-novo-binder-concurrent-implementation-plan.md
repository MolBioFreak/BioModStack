...
Status: final V2 implementation packet, with the focused [sequence/packing corrections](sequence-design-packing-harmonization.md) controlling SQ01–SQ08. Replaces the historical dispatch prompts in this file, including obsolete absence claims and the superseded classification-policy work. Read the controlling [completion specification](de-novo-binder-harmonization.md), [complete acceptance scope](de-novo-binder-design-upgrade.md) and [product outline](de-novo-binder-upgrade-outline.md) together. This packet specifies implementation; it does not claim implementation or native acceptance occurred. Dispatch only the requested correction slice; do not restart the entire binder program for sequence/packing parity.
...
Preserve full global FrustraMPNN and selected Caliby, relevant UI/API settings, masks, runtime inputs, native outputs and workbench. For the focused sequence/packing slice, implement SQ01–SQ04 model-owned corrections rather than treating Caliby as native-testing-only work. D additionally owns `platform/api/config/models/fampnn.yaml`, `proteinmpnn.yaml`, `scripts/prep_mpnn_designs.py`, `modules/proteinmpnn.nf`, `modules/fampnn.nf` and `apptainer/caliby.def` when changed. B retains `antibody_denovo.yaml` and parent workflow composition; parent retains `native_components.py`/compiler/normalization; A retains controls; C retains selection/publication; G retains shared runtime catalog/bridge. The ordinary LigandMPNN SQ06 leaf is D-owned; H retains the diagnostic leaf. Parent alone integrates both slices into the shared `ligandmpnn.yaml`. Assign any genuinely needed new leaf files before editing. B owns SQ07 legacy modules. SQ08 remains a separate product decision, not a shortcut through retired-model enablement. Resolve current FrustraMPNN placement at its own scheduler/bridge owner with G rather than forcing a generic target override; this focused slice does not reopen that wider work. No local reduced analysis or cross-chain reinterpretation.
...
1. **Bind current source and shared seams.** Parent checks current base and exact path owners. For the requested sequence/packing corrections use SQ01–SQ08's current baseline and delivery order, reconcile concurrent initial-round work, and change existing owners only. Do not dispatch older missing-generator claims or broad Project/UI redesign as prerequisites for that slice. For separately authorized wider binder completion, replace blanket engine restrictions only for intended typed contracts and land the necessary request/source/document/Project extensions. B provides native settings/mode contracts; C/parent settle document and fan-out context transport. No broad restart of the completed audit.
