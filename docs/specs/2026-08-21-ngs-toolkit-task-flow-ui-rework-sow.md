...
The amendment permits changes only in the frontend payload helper/component, the existing ONT submit boundary, request normalizer and stage-response path, the Construct Screening workflow, the existing ONT workflow registry, and their focused tests. It does not authorize new workflow families, database migrations, scheduler redesign, scientific computation, retry or provenance changes, or Production deployment.
