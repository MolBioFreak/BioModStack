...
`services.remote_execution.cache_template.render_vm_cache_template_fields` renders the actual guest bootstrap from `tools/bms_artifact_cache.py` and its `helper_url`, an immutable full-commit GitHub raw URL. It downloads at most 2 MiB and verifies the exact supplied helper SHA256 before atomic installation and initialization of `<worker_root>/cache/artifacts/v1`. Helper growth no longer consumes the provider's bounded startup/environment fields. There is no mutable download URL, credential dependency, implicit model download, or job launch.

Ubuntu 22.04's default Python 3.10 is below the worker's Python 3.11 floor. On old x86-64 root-owned VMs only, the startup hook downloads the baseline CPython 3.12.11/20250918 install-only distribution selected by BMS's pinned uv 0.8.22 metadata, verifies SHA256 `d8f71e55f8fd6a4cc9d18ce697969b3326a3325615147e70a9c4b1fa1c3698a8`, and checks the interpreter, TLS and SQLite before publishing a separate prefix under `<worker_root>/runner/`. `/usr/local/bin/python3` makes it available to subsequent SSH attachment; `/usr/bin/python3` is unchanged. Modern guests retain their interpreter. Unexpected existing installation destinations are refused rather than overwritten. This is a base guest prerequisite, not another scientific image or a substitute for the managed support-Python projection. Provisioning requires public HTTPS access to the immutable helper and Python release assets; failures remain visible and do not weaken Attach readiness.
...
On a selected worker, **Workflow → De Novo Design → All De Novo dependencies** prepares the built-in family through the catalog selection `{"kind":"model","model_id":"protein_modification_experimental"}`. The same action is available beside placement in Generate, Redesign structure and Shape, even before a source, geometry or scientific draft is complete. Preview and Start provision remain explicit actions; opening or switching editors does not upload inputs, search MSAs, create a Job or rent a worker.

This full-family selection includes RFD3, DISCO, La-Proteina, Shape, the existing FA-MPNN/ProteinMPNN choices, and Boltz2/ESMFold2/Protenix validators. Shared assets are deduplicated; RFD3 uses its exact checkpoint member, Protenix uses its declared default native members, and image-contained designer weights are not downloaded separately. Custom runtime overrides remain owned by current-request preparation. The existing **current-request** preview is the leaner choice when only one configured task is needed: selecting the family for preload never changes an ordinary launch's stages or assets. Native RFD3 generation/redesign do not gain validation stages, and Shape's skip policy stays skip.
