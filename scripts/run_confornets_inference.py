...
def _normalize_outputs(request: dict[str, Any], raw_dir: Path, output_dir: Path) -> list[dict[str, Any]]:
    ...
    if compute_evaluation and references:
        ...
        landscape_payload = {
            ...
            "workflow": "confornets_experimental",
            "status": "not_computed",
            "reason": "Reference RMSD/landscape metrics require compute_evaluation=true and at least one staged reference; no synthetic landscape values are generated by the wrapper.",
            "sample_count": len(samples),
