...
def test_extract_templates_falls_back_without_biopython(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(extract_target_templates, "_resolve_authoritative_rcsb_mmcif", lambda _path: None)
...
def test_extract_templates_prefers_authoritative_rcsb_mmcif(monkeypatch, tmp_path: Path) -> None:
    pdb_path = Path(__file__).resolve().parent.parent / "rcsb" / "2lgv.pdb"
    cached_cif = tmp_path / "2lgv_downloaded.cif"
    cached_cif.write_text("data_2LGV\n_entry.id 2LGV\n", encoding="utf-8")

    monkeypatch.setattr(extract_target_templates, "_resolve_authoritative_rcsb_mmcif", lambda _path: cached_cif)
    ...
    assert template_info["writer"] == "rcsb"
    assert template_info["chains"] == ["A"]
    assert cif_path.read_text(encoding="utf-8") == cached_cif.read_text(encoding="utf-8")
